...
export default function Home() {
  ...
  const parseJSONResponse = (content: string): { reply: string; options: string[] } => {
    ...
    return {
      reply: content,
      options: ['🤔 你继续说吧，我听着呢', '🎨 换个话题聊聊', '✨ 懒得打字，给我几个选择呗'],
    };
  };

  const buildAPIMessages = (allMessages: Message[], newUserContent: string | ContentItem[]): APIMessage[] => {
  ...
  const fetchInitialOptions = async () => {
    ...
    try {
      setMessages((prev) =>
        prev.map((msg) => (msg.id === initialMessageId ? { ...msg, content: '✨ 正在准备超级有趣的话题...' } : msg))
      );
      ...
      await processStreamResponse(
        ...
        (displayContent) => {
          setMessages((prev) =>
            prev.map((msg) => (msg.id === initialMessageId ? { ...msg, content: displayContent } : msg))
          );
        },
        (finalContent) => {
          const { reply, options } = parseJSONResponse(finalContent);
          setMessages((prev) => prev.map((msg) => (msg.id === initialMessageId ? { ...msg, content: reply } : msg)));
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === initialMessageId ? { ...msg, content: '抱歉，欢迎语加载失败了 😢 但你可以随便聊聊哦！' } : msg
        )
      );
    ...
    try {
      ...
      await processStreamResponse(
        ...
        (displayContent) => {
          ...
          setMessages((prev) =>
            prev.map((msg) => (msg.id === aiMessageId ? { ...msg, content: displayContent } : msg))
          );
        },
        (finalContent) => {
          if (!finalContent) {
            setMessages((prev) =>
              prev.map((msg) => (msg.id === aiMessageId ? { ...msg, content: '抱歉，我无法生成回复。' } : msg))
            );
          } else {
            const { reply, options } = parseJSONResponse(finalContent);

            setMessages((prev) => prev.map((msg) => (msg.id === aiMessageId ? { ...msg, content: reply } : msg)));
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        setMessages((prev) =>
          prev.map((msg) => (msg.id === aiMessageId ? { ...msg, content: '抱歉，连接服务器失败，请稍后再试。' } : msg))
        );
      }
    } finally {
      setIsGenerating(false);
      ...
      return (
        <div>
          {hasComplexMarkdown ? (
            ...
            <div style={{ whiteSpace: 'pre-wrap' }}>{renderTextWithBold(content)}</div>
          )}
          {shouldShowOptions && (
            <div className="message-options">
              ...
              <div className="options-buttons">
                {suggestedOptions.map((option, index) => (
                  <button key={index} className="option-button-in-message" onClick={() => handleOptionClick(option)}>
                    {option}
                  </button>
                ))}
              </div>
          ...
          if (item.type === 'image_url' && item.image_url) {
            return (
              <div key={index} className="uploaded-image-container">
                <Image src={item.image_url.url} alt="上传的图片" width={200} height={150} className="uploaded-image" />
              </div>
            );
          }
          return null;
      ...
      <div className="chat-container">
        <div className="header">
          <div style={{ display: 'inline-block' }}>
            <h1
              className="text-3xl md:text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-red-600 via-green-600 to-red-600 shimmer"
          ...
          <p className="mt-1 text-red-700/90 text-sm glow">
            <span className="emoji-bounce">🎄</span>顶<span className="emoji-bounce">🎅</span>级
            <span className="emoji-bounce">⛄</span>牛<span className="emoji-bounce">🎁</span>马
          ...
          {messages.map((message) => (
            <div key={message.id} className={`message ${message.role}`}>
              <div className="avatar">
                {message.role === 'ai' ? (
                  <Image src="/robot-santa.png" alt="AI助手" width={40} height={40} className="avatar-img" />
                ) : (
                  '🐮'
                )}
              </div>
              ...
              <div className="uploaded-files">
                {uploadedFiles.map((file, index) => (
                  <div key={index} className="file-preview">
                    <Image src={file.data} alt={file.name} width={80} height={80} />
                    <button className="remove-file" onClick={() => removeFile(index)} aria-label="删除图片">
                      ×
                    </button>
                  </div>
                ))}
