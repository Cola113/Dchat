...
function linkSignals(source: AbortSignal | undefined, target: AbortController) {
  if (!source) return () => {};
  const onAbort = () => {
    try { target.abort((source as any).reason); } catch { target.abort(); }
  };
  if (source.aborted) {
    onAbort();
    return () => {};
  ...
  try {
    ...
    const outStream = new ReadableStream<Uint8Array>({
      ...
      cancel() {
        ...
        try { abortController.abort('stream-cancel'); } catch {}
        try { unlink(); } catch {}
      },
    });
...
async function raceProviders(
  ...
  timeoutId = setTimeout(() => {
    ...
    perControllers.forEach(c => { try { c.abort('race-timeout'); } catch {} });
  }, RACE_TIMEOUT_MS);
      ...
      p.then(outcome => {
        ...
        if (outcome.ok) {
          ...
          perControllers.forEach((c, j) => {
            if (j !== outcome.index) {
              try { c.abort('winner-found'); } catch {}
            }
          });
          ...
          console.warn(`[${providers[idx].name}] 竞速失败:`, outcome.error instanceof Error ? outcome.error.message : outcome.error);
          remaining -= 1;
